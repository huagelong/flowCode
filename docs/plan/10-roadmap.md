...
| T19 | 讨论→需求→任务列表 Issue | `/backlog` 指令产出需求 Issue（状态=backlog），`/todo` 基于该需求生成 `parent_id` 归属的任务 Issue（状态=todo），不需要人工确认 |
...
| **前端组件测试** | Vitest + Testing Library | 核心组件（AgentForm / IssueCard / IssueKanban） | PR |
