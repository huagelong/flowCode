...
| 错误码前缀 | 模块 | 示例 |
|-----------|------|------|
| `ERR_AUTH_*` | 认证 | `ERR_AUTH_INVALID_TOKEN` / `ERR_AUTH_EXPIRED` |
| `ERR_PERM_*` | 权限 | `ERR_PERM_DENIED` / `ERR_PERM_ROLE_DRIFT` |
| `ERR_ORG_*` | 组织 | `ERR_ORG_NOT_FOUND` / `ERR_ORG_NAME_EXISTS` |
| `ERR_MEMBER_*` | 成员 | `ERR_MEMBER_ALREADY_EXISTS` / `ERR_MEMBER_INVITE_EXPIRED` |
| `ERR_PROJECT_*` | 项目 | `ERR_PROJECT_GIT_AUTH_FAILED` / `ERR_PROJECT_NOT_FOUND` |
| `ERR_ISSUE_*` | Issue | `ERR_ISSUE_INVALID_TRANSITION` / `ERR_ISSUE_NOT_FOUND` |
| `ERR_AGENT_*` | Agent | `ERR_AGENT_DISABLED` / `ERR_AGENT_LLM_TIMEOUT` |
| `ERR_SANDBOX_*` | 沙箱 | `ERR_SANDBOX_CREATE_FAILED` / `ERR_SANDBOX_CRASHED` |
| `ERR_QUEUE_*` | 队列 | `ERR_QUEUE_FULL` / `ERR_QUEUE_TIMEOUT` |
| `ERR_TOKEN_*` | Token | `ERR_TOKEN_QUOTA_EXCEEDED` / `ERR_TOKEN_ORG_LIMIT` |
| `ERR_GITHUB_*` | GitHub | `ERR_GITHUB_PR_CONFLICT` / `ERR_GITHUB_RATE_LIMITED` |

> 错误码国际化：后端通过 `go-i18n` 根据 `Accept-Language` 头返回对应语言的 `message`，`code` 字段始终为英文不变。
