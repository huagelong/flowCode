...
唯一需要人工确认的节点是 `in_review`。进入该状态时系统创建/展示 PR，并通过群聊消息提醒审核：

```
┌─ 群聊 ──────────────────────────────────────────────────────────────┐
│ 12:00  system   已生成需求 Issue #1「用户登录页需求」(backlog, P1)     │
│ 12:03  system   已分析出 3 个任务，放入 Issue #1 下                    │
│ ─────────────────────────────────────────────────────────────────── │
│ 12:30  system   Issue #2 PR 已提交 (in_review)                      │
│                PR: https://github.com/org/repo/pull/42              │
│                [审核通过] [退回修改] [查看变更]                      │
│ 12:35  张三     在 GitHub 完成 Review 并 Merge                       │
│ 12:35  system   Issue #2 已完成 (done) ✅                            │
│ ─────────────────────────────────────────────────────────────────── │
...
| 决策点 | 群聊动作 | 可选操作 |
|--------|---------|---------|
| PR 待审核 | 系统发送 PR 链接卡片到群聊 | [审核通过] / [退回修改] / [查看变更] |
| 执行失败 | 系统发送失败摘要到群聊 | [查看日志] / [重新执行] / [停止] |
| PR 被拒 | 系统发送被拒原因到群聊 | [补充指令] / [查看变更] / [放弃] |
