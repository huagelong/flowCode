> ???`docs/plan/06-agent.md` ? 129 ?
> ???[???](../../README.md) -> [AnserFlow - anserAgent 智能体系统](../README.md) -> [四、Workflow Agents — 确定性编排](README.md) -> 4.1 SequentialAgent — 顺序执行
> ???[???](README.md) ? [???](02-4.2-ParallelAgent-—-并发执行.md)
> ?????[??????](README.md) ? [??????](../README.md)
